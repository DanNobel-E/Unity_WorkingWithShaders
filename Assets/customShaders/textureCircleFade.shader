...
Shader "Custom/textureCircleFade"
{
    Properties
    {
...
half4 frag(vertexOutput i): SV_TARGET{
    ...
    if(c_uvDist<=pow(_Radius,2)-pow(_Fade,2)){

        finalColor.a=1;
    }else if(c_uvDist<=pow(_Radius,2) && c_uvDist>pow(_Radius,2)-pow(_Fade,2)){


finalColor.a= smoothstep(pow(_Radius,2),pow(_Radius,2)-pow(_Fade,2), c_uvDist);
    }
    else{
        finalColor.a=0;
    }
