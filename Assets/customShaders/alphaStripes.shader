Shader "Custom/alphaStripes"
{
    Properties
    {
        ...
        _StripesCutOff ("StripesHeight", Range(0,1))=0
...
Pass
        {
            ...
            half4 frag(vertexOutput i): SV_Target
            {
                ...
                if(_Stripes>1){
              float4 localPos= i.objPos;

                float stripes= 5/_Stripes*2;
                float mod=  floor(localPos.z/stripes);
                float pos= mod*stripes;
                float width= stripes*_StripesCutOff;



                if(fmod(mod,2)!=0){

                if(localPos.z>=pos && localPos.z<pos+width){
