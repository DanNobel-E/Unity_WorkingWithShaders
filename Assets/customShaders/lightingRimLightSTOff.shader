Shader "Custom/lightingRimLightSTOff"
{
    Properties
    {
        ...
        [KeywordEnum(Off,Vert,Frag, Rim)] _Lighting("LightingMode", Float) = 0
        ...
        _RimBorder("RimBorder", Float) = 10
        _RimCutOff("RimCutOff", Range(0,1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature _LIGHTING_OFF _LIGHTING_VERT _LIGHTING_FRAG _LIGHTING_RIM
            ...
            struct vertexOutput
            {
                ...
                #endif
                #if _LIGHTING_FRAG || _LIGHTING_RIM
            ...
            float3 SpecularRimLight(float3 n, float3 lightDir, float3 V, float3 specularCol, float specularFactor, float specularPower, float attenuation)
            {
                //float3 H = normalize(lightDir + V);

                float absol= 1-abs(dot(n, V));
                
                float p= pow(1-abs(dot(n, V)),_RimBorder);
                ...
                return specularCol * specularFactor * attenuation * p;
            }
            ...
            half4 frag(vertexOutput i): SV_Target
            {
                ...
                float3 normalWorldAtPixel;
                    ...
                    #else
                        float3 specular = SpecularRimLight(normalWorldAtPixel, lightDir, V, specularColor, _Specular, _SpecularPower, attenuation);
                      
                     #endif
                    #if _AMBIENTMODE_ON
                        //ambient
                        float ambient = _Ambient * UNITY_LIGHTMODEL_AMBIENT;
                        finalColor = float4(texColor*diffuse + specular + ambient, specular.x* texColor.a);
                    #else
                        finalColor = float4(texColor*diffuse + specular,specular.x* texColor.a);
                    #endif
